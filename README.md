...
Athlete curve-set `activities` maps contain compact `AthleteCurveActivity` summaries, not full `Activity` objects. Live responses can omit `type` even when the request has a type filter, so treat that field as optional and retain the request filter as provenance.
